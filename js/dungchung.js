...
// Thêm sản phẩm kèm thông số (dùng cho mục nổi bật trên trang chủ)
function addProductWithSpecs(p, ele, returnString) {
    ...
    if (product.rateCount > 0) {
        for (var i = 1; i <= 5; i++) {
            rating += (i <= product.star) ? `<i class="fa fa-star"></i>` : `<i class="fa fa-star-o"></i>`;
        }
    ...
    if (product.promo && product.promo.name == "giareonline") {
        price = `<strong>` + product.promo.value + `&#8363;</strong><span>` + product.price + `&#8363;</span>`;
    }

    // specs (chọn một vài thông số tiêu biểu)
    var d = p.detail || {};
    var specs = `<ul class="specs">`;
    if (d.screen) specs += `<li><strong>Màn hình:</strong> ` + d.screen + `</li>`;
    if (d.cpu) specs += `<li><strong>CPU:</strong> ` + d.cpu + `</li>`;
    if (d.ram) specs += `<li><strong>RAM:</strong> ` + d.ram + `</li>`;
    if (d.battery) specs += `<li><strong>Pin:</strong> ` + d.battery + `</li>`;
    specs += `</ul>`;

    var chitietSp = 'chitietsanpham.html?' + product.name.split(' ').join('-');

    var li =
    `<li class="sanPham spec">
        <a href="` + chitietSp + `">
            <img src=` + product.img + ` alt="">
            <h3>` + product.name + `</h3>
            <div class="price">` + price + `</div>
            <div class="ratingresult">` + rating + `</div>
            ` + (product.promo && product.promo.toWeb()) + `
            ` + specs + `
            <div class="tooltip">
...
function addHeader() {
	...
	<div class="header group">
        <div class="logo">
            <a href="index.html" class="logo-text" title="TSon28">TSon28</a>
        </div> <!-- End Logo -->
...
function addFooter() {
        ...
        <section>
            <div class="contact-grid">
                <div class="contact-item">
                    ...
                    <p><i class="fa fa-phone"></i> 1900 232 460 <span>(8:00 - 21:30)</span></p>
                </div>
                <div class="contact-item">
                    <h4>Chăm sóc khách hàng</h4>
                    <p><i class="fa fa-envelope"></i> support@lddphonestore.vn</p>
                </div>
                <div class="contact-item">
                    <h4>Kết nối</h4>
                    <p class="socials">
    ...
    <div class="copy-right">
        <p><a href="index.html">LDD Phone Store</a> - All rights reserved © 2025 - Designed by
            <span style="color: #eee; font-weight: bold">Nguyễn Thái Sơn</span></p>
    </div>`);
}
...
// Thêm trợ lý AI chatbot
function addAIAssistant() {
    document.write(`
    <div class="ai-assistant" id="aiAssistant">
        <div class="ai-chat-icon" onclick="toggleAIChat()">
            <i class="fa fa-robot"></i>
            <span>24/7 Trợ lý AI</span>
        </div>
        
        <div class="ai-chat-window" id="aiChatWindow">
            <div class="ai-chat-header">
                <span>Trợ lý AI TSon28</span>
                <div>
                    <button class="ai-close-btn" title="Cài đặt" onclick="toggleAISettings()"><i class="fa fa-cog"></i></button>
                    <button class="ai-close-btn" title="Đóng" onclick="toggleAIChat()">×</button>
                </div>
            </div>
            ...
            <div class="ai-settings" id="aiSettings" style="display:none;">
                ...
                <div class="ai-setting-row">
                    <label>Model</label>
                    <input id="aiModel" placeholder="gpt-3.5-turbo | gpt-4o-mini | gemini-1.5-flash">
                </div>
                <div class="ai-setting-row">
                    <label>API Key</label>
                    <input id="aiApiKey" placeholder="Dán API key của bạn vào đây">
                </div>
                <div class="ai-setting-actions">
                    <button class="ai-option" onclick="saveAIConfigUI()">Lưu</button>
                </div>
                <div class="ai-setting-note">Lưu ý: Không nên dùng API key trực tiếp trên client cho môi trường thật. Hãy dùng proxy server để bảo mật khóa.</div>
            </div>
            ...
            <div class="ai-chat-messages" id="aiChatMessages">
                <div class="ai-message">
                    ...
                    <div class="ai-bubble">
                        Xin chào! Tôi là trợ lý AI của TSon28. Hãy nhấn vào biểu tượng bánh răng để nhập API key OpenAI hoặc Gemini và bắt đầu trò chuyện.
                    </div>
                </div>
            </div>
            
            <div class="ai-quick-options">
                <button class="ai-option" onclick="aiQuickResponse('Điện thoại iPhone')">Điện thoại iPhone</button>
                <button class="ai-option" onclick="aiQuickResponse('Điện thoại Android')">Điện thoại Android</button>
                <button class="ai-option" onclick="aiQuickResponse('Laptop')">Laptop</button>
                <button class="ai-option" onclick="aiQuickResponse('Phụ kiện')">Phụ kiện</button>
            </div>
            
            <div class="ai-chat-input">
                <input type="text" id="aiInput" placeholder="Nhập câu hỏi của bạn..." onkeypress="handleAIInput(event)">
...
function toggleAIChat() {
    ...
    const chatIcon = document.querySelector('.ai-chat-icon');
...
function toggleAISettings() {
    ...
    if (isHidden) loadAIConfigUI();
}

function loadAIConfig() {
    const provider = localStorage.getItem('AI_PROVIDER') || 'openai';
    const model = localStorage.getItem('AI_MODEL') || (provider === 'openai' ? 'gpt-3.5-turbo' : 'gemini-1.5-flash');
    const apiKey = localStorage.getItem('AI_API_KEY') || '';
    return { provider, model, apiKey };
}

function loadAIConfigUI() {
    const { provider, model, apiKey } = loadAIConfig();
...
function saveAIConfigUI() {
    ...
    const model = document.getElementById('aiModel').value || (provider === 'openai' ? 'gpt-3.5-turbo' : 'gemini-1.5-flash');
    const apiKey = document.getElementById('aiApiKey').value.trim();
    localStorage.setItem('AI_PROVIDER', provider);
    localStorage.setItem('AI_MODEL', model);
    if (apiKey) localStorage.setItem('AI_API_KEY', apiKey);
    addAIMessage('ai', 'Đã lưu cài đặt. Bạn có thể bắt đầu trò chuyện.');
}

// Xử lý tin nhắn nhanh
function aiQuickResponse(option) {
    addAIMessage('user', option);
    
    let response = '';
    switch(option) {
        case 'Điện thoại iPhone':
            response = 'Chúng tôi có nhiều mẫu iPhone từ iPhone 12 đến iPhone 15 Pro Max với nhiều mức giá khác nhau. Bạn muốn tìm iPhone nào?';
            break;
        case 'Điện thoại Android':
            response = 'Chúng tôi có đầy đủ các thương hiệu: Samsung, Xiaomi, Oppo, Huawei... Bạn thích thương hiệu nào?';
            break;
        case 'Laptop':
            response = 'Chúng tôi có laptop gaming, laptop văn phòng từ các thương hiệu uy tín. Bạn cần laptop để làm gì?';
            break;
        case 'Phụ kiện':
            response = 'Chúng tôi có tai nghe, sạc dự phòng, ốp lưng, cáp sạc... Bạn cần phụ kiện gì?';
            break;
        default:
            response = 'Tôi có thể giúp bạn tìm kiếm sản phẩm. Hãy cho tôi biết bạn cần gì nhé!';
    }
...
function sendAIMessage() {
    ...
    if (message) {
        ...
        input.value = '';
        aiSendToProvider(message).catch(err => {
            addAIMessage('ai', 'Không thể liên hệ dịch vụ AI: ' + (err && err.message ? err.message : 'Lỗi không xác định'));
        });
    }
}
...
async function aiSendToProvider(userText) {
    window.__aiChatHistory = window.__aiChatHistory || [];
    const cfg = loadAIConfig();
    if (!cfg.apiKey) {
        addAIMessage('ai', 'Bạn chưa nhập API key. Nhấn biểu tượng bánh răng để cấu hình.');
        return;
    }
    __aiChatHistory.push({ role: 'user', content: userText });
    if (cfg.provider === 'openai') {
        const reply = await callOpenAI(cfg, __aiChatHistory);
        __aiChatHistory.push({ role: 'assistant', content: reply });
        addAIMessage('ai', reply);
    } else if (cfg.provider === 'gemini') {
        const reply = await callGemini(cfg, __aiChatHistory);
        __aiChatHistory.push({ role: 'assistant', content: reply });
        addAIMessage('ai', reply);
    } else {
        addAIMessage('ai', 'Nhà cung cấp không hợp lệ.');
    }
}

async function callOpenAI(cfg, history) {
    const messages = [{ role: 'system', content: 'Bạn là trợ lý mua sắm của cửa hàng TSon28. Trả lời ngắn gọn, thân thiện, tiếng Việt.' }];
    ...
    return content || 'Xin lỗi, tôi chưa có phản hồi.';
}

async function callGemini(cfg, history) {
    const contents = history.map(m => ({ role: m.role === 'assistant' ? 'model' : 'user', parts: [{ text: m.content }] }));
    const body = { contents };
    const endpoint = 'https://generativelanguage.googleapis.com/v1beta/models/' + (cfg.model || 'gemini-1.5-flash') + ':generateContent?key=' + encodeURIComponent(cfg.apiKey);
    ...
    return text || 'Xin lỗi, tôi chưa có phản hồi.';
}
