window.onload = function () {
	...
	if (filters.length) { // có filter
		...
		// Tạo một khung tuỳ biến: CÔNG NGHỆ NỔI BẬT (robot, iPad, flagship)
		try {
			var tieuBieu = [];
			var tenCanLay = [
				'Samsung Galaxy S24 Ultra 512GB',
				'iPhone 15 Pro Max 256GB',
				'iPad Pro 11 (2024) Wi‑Fi 256GB',
				'Xiaomi Pad 6 8GB/256GB',
				'Xiaomi Robot Vacuum X10+'
			];
			for (var i = 0; i < list_products.length; i++) {
				if (tenCanLay.indexOf(list_products[i].name) >= 0) tieuBieu.push(list_products[i]);
			}

			if (tieuBieu.length) {
				var gradient = 'background-image: linear-gradient(120deg, #42bcf4 0%, #004c70 50%, #42bcf4 100%);';
				var borderColor = 'border-color: #42bcf4';
				var s = `<div class="khungSanPham" style="` + borderColor + `">
						<h3 class="tenKhung" style="` + gradient + `">* CÔNG NGHỆ NỔI BẬT *</h3>
						<div class="listSpTrongKhung flexContain">`;
				for (var j = 0; j < tieuBieu.length; j++) {
					s += addProductWithSpecs(tieuBieu[j], null, true);
				}
				s += `</div></div><hr>`;
				div.innerHTML = s + div.innerHTML; // đưa khung này lên trên cùng
			}
